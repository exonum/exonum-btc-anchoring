...
fn test_anchoring_second_block_transfer_config() {
    ...
    let (_, signatures) = anchoring_state.gen_anchoring_tx_with_signatures(&sandbox,
                                          ...
                                          2000);
    let transfer_tx = anchoring_state.latest_anchored_tx();
    debug!("from_addr={}", anchoring_state.genesis.multisig().address);
    debug!("to_addr={}", following_multisig.address);
    debug!("transfer_tx={:#?}, addr={}",
           transfer_tx,
           transfer_tx.output_address(following_cfg.network()).to_base58check());
