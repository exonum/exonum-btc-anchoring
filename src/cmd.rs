...
impl CommandExtension for GenerateNodeConfig {
    fn args(&self) -> Vec<Argument> {
        vec![
            ...
            "Host of bitcoind.", None, "anchoring-host", false),
            Argument::new_named("ANCHORING_RPC_USER", false,
            "User to login into bitcoind.",  None, "anchoring-user", false),
            Argument::new_named("ANCHORING_RPC_PASSWD", false,
            "Password to login into bitcoind.",  None, "anchoring-password", false),
        ]
    }
    ...
    fn execute(&self, mut context: Context) -> Result<Context, Box<Error>> {
        ...
        let network: String = config.services_config.get("anchoring_network")
        ...
        let mut services_public_configs: BTreeMap<String, Value> = 
            context.get("services_public_configs")
                .unwrap_or_default();
        services_public_configs.extend(vec![("anchoring_pub_key".to_owned(), 
        ...
        let mut services_secret_configs: BTreeMap<String, Value> = 
            context.get("services_secret_configs")
                .unwrap_or_default();
...
impl CommandExtension for GenerateCommonConfig {
    fn args(&self) -> Vec<Argument> {
        vec![
            ...
            "The frequency of anchoring in blocks", None, "anchoring-frequency", false),
            Argument::new_named("ANCHORING_UTXO_CONFIRMATIONS", false,
            "The minimum number of confirmations for anchoring transactions", None, "anchoring-utxo-confirmations", false),
            Argument::new_named("ANCHORING_FEE", true,
            "Fee that anchoring nodes should use.",  None, "anchoring-fee", false),
...
impl CommandExtension for Finalize {
    fn args(&self) -> Vec<Argument> {
        vec![
            ...
            "Txid of the initial funding tx", None, "anchoring-funding-txid", false),
            Argument::new_named("ANCHORING_CREATE_FUNDING_TX", false, 
            "Create initial funding tx with given amount in satoshis",
            None, "anchoring-create-funding-tx", false)]
    }

    fn execute(&self, mut context: Context) -> Result<Context, Box<Error>> {
            let mut node_config: NodeConfig = context.get("node_config").unwrap();
            let common_config: CommonConfigTemplate
                                = context.get("common_config").unwrap();
            let public_config_list: Vec<NodePublicConfig>
                                = context.get("public_config_list").unwrap();
            ...
            let create_funding_tx_with_amount =
                            context.arg::<u64>("ANCHORING_CREATE_FUNDING_TX").ok();
            ...
            let rpc: AnchoringRpcConfig = services_secret_configs
                            ...
                            .try_into()
                            .unwrap();
            let network: String = common_config.services_config
                            .get("anchoring_network")
                            .expect("Anchoring network not fount")
                            .clone()
                            .try_into()
                            .unwrap();
            let utxo_confirmations: u64 = common_config.services_config
                            .get("anchoring_utxo_confirmations")
                            .expect("Anchoring utxo confirmations not fount")
                            .clone()
                            .try_into()
                            .unwrap();
            let frequency: u64 = common_config.services_config
                            .get("anchoring_frequency")
                            .expect("Anchoring frequency not fount")
                            .clone()
                            .try_into()
                            .unwrap();
            let fee: u64 = common_config.services_config
                            .get("anchoring_fee")
